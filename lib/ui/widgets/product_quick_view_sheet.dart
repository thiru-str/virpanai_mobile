...
class ProductQuickViewSheet extends StatefulWidget {
  ...
  const ProductQuickViewSheet({Key? key, required this.productId}) : super(key: key);
...
class _ProductQuickViewSheetState extends State<ProductQuickViewSheet> {
          ...
          children: [
            Container(
              ...
              child: product == null
                ...
                padding: const EdgeInsets.only(bottom: 100), // Matches button height
                child: Column(
  ...
  Widget buildPrice() {
    final price = selectedVariant?.calculatedPrice?.rawCalculatedAmount?.value ?? '0';
    return Text(
  ...
  Widget buildAddToCartButton() {
    return ElevatedButton(
      onPressed: selectedVariantId == null
          ...
          : () async {
        final apiService = ApiService();
        await apiService.addCart(context, selectedQuantity, selectedVariantId!);
        final cartResponse = await apiService.getCart(context);
        eventBus.fire(ViewCartModel(
          cartResponse.cart!.items!.length,
          cartResponse.cart!.items!.map((i) => i.thumbnail!).toList(),
        ));
